...
console.log(this.cards);
    ...
    this.cards = Array.from(this.cards).map( x => this.cards());
  ...
  selectTab(){
    ...
    tabs.forEach(tabList => tabList.classList.remove('.active-tab'));

    // Select all of the elements with the .card class on them
    const cards = document.querySelectorAll('.card');

    // Iterate through the NodeList setting the display style each one to 'none'
    cards.forEach(tabList => tabList.classList.style.display = "none");
    
    // Add a class of ".active-tab" to this.tabElement
    this.tabElement.classList.add('.active-tab');
  
    // Notice we are looping through the this.cards array and invoking selectCard() from the
    // TabCard class. Just un-comment the code and study what is happening here.
